"""SVG renderer helpers for kakao_map LLM tools.
...
def svg_table(
    ...
    *,
    ...
    parts.append(f'<rect width="{width}" height="{height}" rx="14" ry="14" fill="{_BG}"/>')
    # Accent header band
    parts.append(
    ...
    if not rows:
        ...
        for ri, row in enumerate(rows):
            ...
            if ri % 2 == 0:
                parts.append(
                    f'<rect x="0" y="{ry}" width="{width}" height="{row_h}" fill="{_BG_ALT}"/>'
                )
            x = pad_x
...
def svg_card(
    ...
    *,
    ...
    parts.append(f'<rect width="{width}" height="{height}" rx="16" ry="16" fill="{_BG}"/>')
    parts.append(
...
def grid_results(
    items: Iterable[tuple[str, list[tuple[str, str]], dict[str, Any] | None]],
    *,
    accent: str = _DEFAULT_ACCENT,
) -> list[dict[str, str]]:
