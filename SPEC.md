...
각 지점(출발/도착)은 **entity selector 또는 좌표** 중 하나로 입력. 둘 다 또는 둘 다 아님 → 에러.

| 필드 | selector | 필수 | 설명 |
|---|---|---|---|
| `origin_entity` | `entity` (device_tracker/person/zone 등 위치 보유 도메인) | △ | 출발지 엔티티 |
| `origin_coords` | list[float] — `[위도, 경도]` | △ | 출발지 좌표 (`origin_entity`와 택1) |
| `destination_entity` | `entity` | △ | 도착지 엔티티 |
| `destination_coords` | list[float] — `[위도, 경도]` | △ | 도착지 좌표 (`destination_entity`와 택1) |
| `waypoints` | list[text] | X | 경유지 최대 5개. 각 항목: entity_id 또는 `"위도,경도"` |
| `mode` | select | X | `car`(기본) / `traffic` / `walk` / `bicycle` |

**위치 해석 규칙:**
1. `*_entity` / waypoint 항목이 entity_id 형태 → 상태의 `latitude`/`longitude` 속성 사용,
   이름은 friendly_name. 좌표 속성 없으면 `ServiceValidationError` (해당 엔티티 명시)
2. `*_coords`는 float 2개 리스트 `[위도, 경도]` (voluptuous:
   `vol.All([vol.Coerce(float)], vol.Length(min=2, max=2))`), waypoint 항목이 `"위도,경도"` 문자열이면
   float 쌍으로 파싱 → 이름은 "출발지"/"도착지"/"경유지N"
